...
Исходя из [документации nginx](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/) можно сделать вывод, что для балансировки ~ 400 000 RPS (сумма запросов от сделок + запросы на получение фронтенд (сборка фронтенда с учетом минификации и сжатия не превышает 2 Мбайта)) по HTTPS нам потребуется 2 сервера данной конфигурации:

|CPUs|Memory|SSD|
|-|-|-|
|4|16 Gbs|128 Gbs|

Сделаем 25% запас мощности и возьмем 3 сервера данной конфигурации.
...
|-|-|-|-|-|-|
|Фронтенд + балансировка|3|Франкфурт|1|1|1|
